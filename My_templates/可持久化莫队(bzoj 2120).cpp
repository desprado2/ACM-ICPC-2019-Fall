...
int main()
{
	...
	for (i=1; i<=cnt2; i++)
	{
		while (T>q[i].tim) {p=c[T].pos; if (p>=L&&p<=R)chk(--cnt[col[p]],'M'); col[p]=init[p]; if (p>=L&&p<=R)chk(++cnt[col[p]],'P'); T--;}
		while (T<q[i].tim) {T++; p=c[T].pos; if (p>=L&&p<=R)chk(--cnt[col[p]],'M'); col[p]=c[T].t; if (p>=L&&p<=R)chk(++cnt[col[p]],'P');}
